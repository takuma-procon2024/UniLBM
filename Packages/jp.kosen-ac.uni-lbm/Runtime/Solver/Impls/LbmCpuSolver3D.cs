...
namespace Solver.Impls
{
    ...
    public class LbmSpuSolver3DImpl
    {
        ...
        public void Update()
        {
            Eval(id => AddSourceVelocity(in id, Time.deltaTime));
            Eval(id => DiffuseVelocity(in id, Time.deltaTime));
            Eval(id => ProjectStep1(in id));
            Eval(id => ProjectStep2(in id));
            Eval(id => ProjectStep3(in id));
            Eval(id => SwapVelocity(in id));
            Eval(id => AdvectVelocity(in id, Time.deltaTime));
            Eval(id => ProjectStep1(in id));
            Eval(id => ProjectStep2(in id));
            Eval(id => ProjectStep3(in id));
            ...
            void Eval(in Action<uint3> action)
            {
                for (var x = 0; x < _width; x++)
                for (var y = 0; y < _height; y++)
                for (var z = 0; z < _depth; z++)
                    action(new uint3((uint)x, (uint)y, (uint)z));
            }
        }
